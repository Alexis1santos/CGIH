...
// Posiciones de las 4 luces de esquina
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform3f(glGetUniformLocation(lightingShader.Program, "spotLight.ambient"), 0.9f, 0.1f, 0.1f);
		...
		for (GLuint i = 0; i < 4; i++)
		{
			...
			glm::vec3 lampColor;
			if (i == 0) lampColor = glm::vec3(1.0f, 1.0f, 1.0f); // cubo 1 blanco
			else if (i == 1) lampColor = glm::vec3(1.0f, 0.0f, 0.0f); // Rojo
			else if (i == 2) lampColor = glm::vec3(0.0f, 1.0f, 0.0f); // Verde
			else lampColor = glm::vec3(0.0f, 0.0f, 1.0f); // azul

			// Enviamos el color al shader de la lámpara
			glUniform3fv(glGetUniformLocation(lampShader.Program, "lightColor"), 1, glm::value_ptr(lampColor));
